...
import sys, FileManager #sys TO CLOSE THE GUI IF AN FATAL ERROR HAPPENS,

#FileManager CLASS IS IMPORTED BECAUSE IT HAS FUNCTIONS FOR ERROR AND WARNING POP-UPS
...
class Super_Tk(Tk):
    ...
    def __init__(self, title = "", icon_path = "", width = 500, height = 500, is_resizable = False):
        ...
        if isinstance(title,str):
            ...
            FileManager.show_error("TypeError", f'"title" must be of type str, not of type {type(title)}.')
            sys.exit(1)
        ...
        if isinstance(is_resizable,bool):
            ...
            FileManager.show_error("TypeError", f'"is_resizable" must be of bool type, not of type {type(is_resizable)}.')
            sys.exit(1)
        ...
        if isinstance(icon_path,str):
            if len(icon_path) > 0:
                try:
                    ...
                    FileManager.show_error("Fatal Error", tkerror)
...
if __name__ == "__main__":
    ...
    FileManager.show_warning("Import Warning", "This script containts a modified version " \
